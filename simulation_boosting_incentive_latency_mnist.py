...
gamma_list = [1000*i for i in range(1, 50)]
gamma_list2 = [10000*i*10 for i in range(1, 18)]
gamma_list.extend(gamma_list2)
...
results_file_name = './results/mnist-ict-boosting'+'_'+str(datasize_method)+'_'+str(reward_method) +'.csv'
...
for r in range(len(rate_list)):
    ...
    for round in range(len(gamma_list)):
        ...
        with open(results_file_name, 'a') as f:
            f.write(str(rate_mean_list[r]) + ',' + str(gamma)+ ',' + str(total_server_payoff) + ',' + str(n_join) + ',' + str(best_data_size) + ',' + str(delay) + ',' + str(el_accu) + ',' + str(diversity) + ',' + str(num_iter) + '\n')
            f.close()
...
plot_2d_mul_lines(x, num_convergence_iter_list, r'$\gamma$','# iterations', linestyle_list, marker_list, label_list,'./results/mnist-ict-cov-boosting'+file_name, y_major=1, x_major=500000)

plot_2d_mul_lines(x, num_joint_list, r'$\gamma$','# participating learners', linestyle_list, marker_list, label_list,'./results/mnist-ict-num-learner-boosting'+file_name, x_major=500000)

plot_2d_mul_lines(x,data_size_learner_list,r'$\gamma$','Data size', linestyle_list, marker_list, label_list, rst='./results/mnist-ict-el-d-boosting'+file_name, x_major=500000)

plot_2d_mul_lines(x,total_delay_list,r'$\gamma$','Latency', linestyle_list, marker_list, label_list,'./results/mnist-ict-delay-boosting'+file_name, x_major=500000)#,xlim=(1,6000))

plot_2d_mul_lines(x,EL_diversity_list,r'$\gamma$','Diversity', linestyle_list, marker_list, label_list,'./results/mnist-ict-div-boosting'+file_name, x_major=500000)#,xlim=(1,6000))

plot_2d_mul_lines(x,EL_accuracy_list,r'$\gamma$','Ensemble accuracy', linestyle_list, marker_list, label_list,'./results/mnist-ict-elaccu-boosting'+file_name, y_major=0.01, ylim=(0.93, 0.96), x_major=500000)#,xlim=(1,6000))

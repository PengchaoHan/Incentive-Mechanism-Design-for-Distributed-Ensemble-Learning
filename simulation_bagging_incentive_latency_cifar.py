...
# gamma_list = [1000*i*1 for i in range(1, 400)]
# gamma_list = [1000*i for i in range(2, 200)]

# gamma_list = [1000*i for i in range(2, 200)]
gamma_list = [1000*i for i in range(5, 200)]
...
#                     0.097812298684595, 0.593434375934323, -1.1861713921636314, 2522.9784618367407, -1.4699885825297731e-06) # todo:change to cifar,currently it is for mnist
#     avg_accu = 0.0
#     for i in range(int(n)):
#         avg_accu += func_accu(d[i], -0.007833609685973542, 1.0247048094739293, -1.3091749146369298, 0.42491097996851623,
#                               0.10497567453740748, 5.4841480479782305, -291.50850212157417, -6.116287113444875,
#                               2.5528317530269358)
#     avg_accu /= n
#     return avg_accu
...
def func_el_accu_real_(data):
    n, d = data  # x: N, y: D
    d = np.mean(d)
    ensemble_accuracy_array = func_el_accu_real((n,d), -0.007833609685973542, 1.0247048094739293, -1.3091749146369298, 0.42491097996851623,
                              0.10497567453740748, 5.4841480479782305, -291.50850212157417, -6.116287113444875,
                              2.5528317530269358)
    return ensemble_accuracy_array
...
def plot_2d_mul_lines(x,y,xlabel,ylabel,linestyle,marker,label,rst,xlim=None,ylim=None, y_major=None,x_major=None,rolling=None):
    fig = plt.figure(figsize=(4, 3))
...
results_file_name = './results/cifar-ict-bagging'+'_'+str(datasize_method)+'_'+str(reward_method) +'.csv'
...
for r in range(len(rate_list)):
    ...
    for round in range(len(gamma_list)):
        ...
        while num_iter < 10 and (num_iter == 0 or total_server_payoff != total_server_payoff_old):
            ...
            for n in range(num_of_base_learners):
                ...
                for m in range(num_of_base_learners):
                    if m != learner_index and learner_decision[m] == 1:
                        ...
                        accu = func_el_accu_real((n_join,data_size[m]), -0.007833609685973542, 1.0247048094739293, -1.3091749146369298, 0.42491097996851623,
                              0.10497567453740748, 5.4841480479782305, -291.50850212157417, -6.116287113444875,
                              2.5528317530269358)
                        # func_accu(data_size[m], -0.007833609685973542, 1.0247048094739293, -1.3091749146369298, 0.42491097996851623,
                        #       0.10497567453740748, 5.4841480479782305, -291.50850212157417, -6.116287113444875,
                        #       2.5528317530269358)
                ...
                accu_candidate = func_el_accu_real((n_join,d_candidate), -0.007833609685973542, 1.0247048094739293, -1.3091749146369298, 0.42491097996851623,
                              0.10497567453740748, 5.4841480479782305, -291.50850212157417, -6.116287113444875,
                              2.5528317530269358)
                # func_accu(d_candidate, -0.007833609685973542, 1.0247048094739293, -1.3091749146369298, 0.42491097996851623,
                #               0.10497567453740748, 5.4841480479782305, -291.50850212157417, -6.116287113444875,
                #               2.5528317530269358)
                ...
                if n_join == 1:
                    ...
                    el_accu_candidate = func_el_accu_real((n_join,d_avg_candidate), -0.007833609685973542, 1.0247048094739293, -1.3091749146369298, 0.42491097996851623,
                              0.10497567453740748, 5.4841480479782305, -291.50850212157417, -6.116287113444875,
                              2.5528317530269358)
                    ...
                    if n_join == 2:
                        ...
                        el_accu_wo_n = func_el_accu_real((n_join-1,d_sum_except_n / (n_join-1)), -0.007833609685973542, 1.0247048094739293, -1.3091749146369298, 0.42491097996851623,
                              0.10497567453740748, 5.4841480479782305, -291.50850212157417, -6.116287113444875,
                              2.5528317530269358)
            ...
            total_server_payoff += gamma * el_accu_real
            for m in range(num_of_base_learners):
                if learner_decision[m] == 1:
            ...
            print("iter" + str(num_iter) + " total server payoff: " + str(total_server_payoff) + " # learners:" + str(
                n_join)  #+ " diversity:" + str(diversity)
                  + " accuracy:" + str(el_accu_real))
        ...
        EL_accuracy.append(el_accu_real)  # el_accu_real
        total_delay.append(delay)
        total_payoff.append(total_server_payoff)

        with open(results_file_name, 'a') as f:
            f.write(str(rate_mean_list[r]) + ',' + str(gamma) + ',' + str(total_server_payoff) + ',' + str(n_join) + ',' + str(np.sum(data_size)/n_join) + ',' + str(delay) + ',' + str(el_accu_real) + ',' + str(num_iter) + '\n')
            f.close()
...
# plot_2d_mul_lines(x, num_convergence_iter_list, r'$\gamma$','# iterations', linestyle_list, marker_list, label_list,'./results/cifar-ict-cov-bagging.pdf', y_major=2)

plot_2d_mul_lines(x, num_joint_list, r'$\gamma$','# participating learners', linestyle_list, marker_list, label_list,'./results/cifar-ict-num-learner-bagging'+file_name,x_major=50000)

# plot_2d_mul_lines(x,data_size_learner_list_1,r'$\gamma$','Data size', linestyle_list, marker_list, label_list, rst='./results/cifar-ict-el-d1-bagging.pdf')
# plot_2d_mul_lines(x,data_size_learner_list_2,r'$\gamma$','Data size', linestyle_list, marker_list, label_list, rst='./results/cifar-ict-el-d2-bagging.pdf')
plot_2d_mul_lines(x,data_size_learner_list_a,r'$\gamma$','Data size', linestyle_list, marker_list, label_list, rst='./results/cifar-ict-el-da-bagging'+file_name,x_major=50000)

plot_2d_mul_lines(x,total_delay_list,r'$\gamma$','Latency', linestyle_list, marker_list, label_list,'./results/cifar-ict-delay-bagging'+file_name,x_major=50000)#,xlim=(1,6000))

# plot_2d_mul_lines(x,EL_diversity_list,r'$\gamma$','Diversity', linestyle_list, marker_list, label_list,'./results/cifar-ict-div-bagging.pdf')#,xlim=(1,6000))

plot_2d_mul_lines(x,EL_accuracy_list,r'$\gamma$','Ensemble accuracy', linestyle_list, marker_list, label_list,'./results/cifar-ict-elaccu-bagging'+file_name, y_major=0.05,x_major=50000)#,xlim=(1,6000))
